import { Habilidad } from "../models/habilidad";

export const TG = {
  ANGULAR: new Habilidad("angular.png", "Angular"),
  SPRINGBOOT: new Habilidad("spring.png", "Spring Boot"),
  JAVASCRIPT: new Habilidad("js.png", "JavaScript"),
  TYPESCRIPT: new Habilidad("ts.png", "TypeSCript"),
  JAVA: new Habilidad("java.png", "Java"),
  REACTJS: new Habilidad("react.png", "React JS"),
  GIT: new Habilidad("git.png", "Git"),
  GITHUB: new Habilidad("gitHub.png", "GitHub"),
  GITLAB: new Habilidad("gitLab.png", "GiTLab"),
  AZURE: new Habilidad("azure.png", "Azure DevOps"),
  HIBERNATE: new Habilidad("hibernate.png", "Hibernate (ORM)"),
  SEQUELIZE: new Habilidad("sequelize.png", "Sequelize (ORM)"),
  MYSQL: new Habilidad("mySql.png", "MySql"),
  SQLSERVER: new Habilidad("sqlServer.png", "Microsoft SQL Server"),
  POSTGRESQL: new Habilidad("postgreSql.png", "PostgreSql"),
  NODEJS: new Habilidad("node.png", "Node JS"),
  EXPRESSJS: new Habilidad("express.png", "Express JS"),
  MICROSERVICIOS: new Habilidad("microservicios.png", "Microservicios"),
  POSTMAN: new Habilidad("postman.png", "Postman"),
  APRIREST: new Habilidad("ApiRest.png", "Api Rest")
}
